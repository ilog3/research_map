...
export function sanitizeThoughtLinesForUser(lines: string[]): string[] {
  ...
  let framingDiagAdded = false;
  let discoveryDiagAdded = false;
  ...
  for (const raw of lines) {
    ...
    for (const seg of segments) {
      ...
      for (let line of splitLines) {
        ...
        if (/^Framing 输出回执[：:]/i.test(line)) {
          const rest = line.replace(/^Framing 输出回执[：:]\s*/i, '').trim();
          if (rest && rest.length < 220 && !/^\{/.test(rest)) {
            pushUnique(out, seen, rest);
          }
          continue;
        }
        if (/^Discovery 输出回执[：:]/i.test(line)) {
          const rest = line.replace(/^Discovery 输出回执[：:]\s*/i, '').trim();
          if (rest && rest.length < 280 && !/^\{/.test(rest)) {
            pushUnique(out, seen, rest);
          }
          continue;
        }
        if (/^Framing 诊断[：:]/i.test(line)) {
          if (!framingDiagAdded) {
            framingDiagAdded = true;
            pushUnique(out, seen, '已完成任务卡格式校验。');
          }
          continue;
        }
        if (/^Discovery 诊断[：:]/i.test(line)) {
          if (!discoveryDiagAdded) {
            discoveryDiagAdded = true;
