...
interface AppState {
  ...
  /** 右栏 tools 优先显示该条助手消息的思考（点击回复时设置） */
  ...
  setSelectedAssistantMessageId: (messageId: string | null) => void;
  ...
  /**
   * 新一条助手回复开始生成时：清空当前线程的检索候选、相关工作、编排/思考轨迹等「实时」侧栏数据，
   * 避免仍显示上一条回复的内容。点击历史助手消息时由该消息的 panelSnapshot 覆盖展示。
   */
  resetLivePanelForNewAssistantTurn: () => void;
...
/** 阅读侧栏完整顺序：前 4 项为主标签，其余在「更多」内折叠（含思考过程，与对话区 reasoning 同步） */
...
export const useStore = create<AppState>((set, get) => ({
  ...
  setSelectedAssistantMessageId: (messageId) => set({ selectedAssistantMessageId: messageId }),
  ...
  resetLivePanelForNewAssistantTurn: () =>
    set((state) => {
      const id = state.activeThreadId;
      const clearedDiscovery = {
        candidatePool: [] as DiscoveryCandidateItem[],
        evidenceList: [] as string[],
        topicClusters: [] as string[],
      };
      const clearedRelated: RelatedWorkOutput = {
        items: [],
        gaps: [],
        summary: '',
        updatedAt: 0,
      };
      const clearedRun: AgentRunState = {
        runId: '',
      ...
      const discoveryByThread = { ...state.discoveryByThread, [id]: clearedDiscovery };
      const relatedWorkByThread = { ...state.relatedWorkByThread, [id]: clearedRelated };
      const agentRunByThread = { ...state.agentRunByThread, [id]: clearedRun };
      ...
      return {
        discoveryCandidatePool: clearedDiscovery.candidatePool,
        discoveryEvidenceList: clearedDiscovery.evidenceList,
        discoveryTopicClusters: clearedDiscovery.topicClusters,
        discoveryByThread,
        relatedWork: clearedRelated,
        relatedWorkByThread,
        agentRunState: clearedRun,
        agentRunByThread,
        retrievalPreviewMeta: null,
        retrievalPreviewByThread: {
          ...state.retrievalPreviewByThread,
        ...
        rightPanelFocusStage: null,
        personalKbWorkbench:
          state.activeAgent === 'personal_kb' ? null : state.personalKbWorkbench,
      };
    }),
    ...
    set((state) => {
      ...
      if (!inReading) {
        const out = {
          ...
          chatThreads,
          lastReadingRightPanelByThread: lastReadingPanels,
          readingSuspendedSnapshot: null as ReadingSuspendedSnapshot | null,
          selectedAssistantMessageId: null,
        };
        const workspaceByThread = {
          ...state.workspaceByThread,
          [id]: buildWorkspaceSnapshot({ ...state, ...out }),
        };
        persistWorkspaceByThread(workspaceByThread);
        return { ...out, workspaceByThread };
      }
  ...
  resumeSuspendedReading: () =>
    set((state) => {
      ...
      const rightPanelCards = Array.from(
        new Set<RightPanelCard>([...snap.rightPanelCards, 'tools'])
      );
      const rightPanelByThread = {
        ...state.rightPanelByThread,
      ...
      const chatThreads = state.chatThreads
        .map((t) =>
          t.id === state.activeThreadId
            ? { ...t, agent: snap.activeAgent, updatedAt: Date.now() }
            : t
        )
        .sort((a, b) => b.updatedAt - a.updatedAt);
      persistChatHistory(chatThreads, state.activeThreadId);
      ...
      const workspaceByThread = {
        ...state.workspaceByThread,
        [state.activeThreadId]: buildWorkspaceSnapshot({ ...state, ...out }),
      };
      persistWorkspaceByThread(workspaceByThread);
      return { ...out, workspaceByThread };
    }),
